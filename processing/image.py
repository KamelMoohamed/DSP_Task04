...
class Image:
    ...
    def crop(self, shape):
        ...
        if ratio>1:
            ...
            dim = (width, height)
            self.img_data = cv.resize(self.img_data, dim, interpolation = cv.INTER_AREA)
        
        axis1= abs(shape[0]- self.img_data.shape[0])//2
        axis2= abs(shape[1]- self.img_data.shape[1])//2
        self.img_data = self.img_data[axis1:axis1+shape[0],axis2:axis2+shape[1]]
